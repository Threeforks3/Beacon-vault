...
[¹] **USCG Rescue 21:** SAROPS drift model centers on the reported vessel position. If the vessel is an empty kayak and the victim is a half-mile upwind, the search pattern is optimized for the wrong target. [[USCG Rescue 21 and search patterns]]
...
[⁴] **USCG Sea School — Datum Marker:** Coxswain SAR doctrine: the strobe IS the datum marker. A victim with a strobe in a separation scenario becomes their own reference point — the SAROPS drift model now works from the correct position because the signal is on the person, not the empty hull. [[USCG Sea School - Coxswain SAR Reference Guide]]
