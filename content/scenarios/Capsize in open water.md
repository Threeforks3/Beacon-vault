...
| EPIRB/PLB | COSPAS-SARSAT² | 406 MHz satellite alert with GPS | Must be activated manually or water-activated on vessel — if vessel sinks, beacon sinks with it |
...
SOS strobe at maximum brightness. 360° hemispherical dome — visible from search aircraft above and surface vessels at water level. IP68 waterproof to 2 meters immersion. Enclosure displacement exceeds device weight — floats lens-up if torn from PFD. Bar-based attachment threads onto PFD webbing — closed-bar design means webbing tears before bars release. 26+ hour runtime spans multi-night operations. IR mode for NVG-equipped search aircraft.
...
[²] **COSPAS-SARSAT:** EPIRBs are vessel-mounted. If the vessel sinks with the EPIRB still in its bracket, no alert is generated. Personal PLBs must be manually activated — a conscious act by a crew member in the water. [[COSPAS-SARSAT]]
